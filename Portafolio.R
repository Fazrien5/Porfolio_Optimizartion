################################################
# Con este código se pueden gestionar y optimizar portafololios de inversión en acciones
#
# Hecho por Mauricio Montiel el 17/04/2020
###############################################
...
chartSeries(gruma, type = "auto",subset = NULL, show.grid = TRUE, name = NULL,time.scale = NULL,
            ...
            theme = chartTheme("black"), layout = NA,major.ticks='auto', minor.ticks=TRUE, yrange=NULL,
            plot=TRUE, color.vol = TRUE, multi.col = F,)

# Caluclo de rendimientos
...
precios <- NULL
for(i in nombres){
  precios <- cbind(precios, getSymbols.yahoo(i, from=fecha, periodicity = "daily", auto.assign=F)[,6])
}
...
ipc <- getSymbols.yahoo("^MXX", from=fecha, periodicity = "daily", auto.assign=F)[,6]
